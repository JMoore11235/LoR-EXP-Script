...
;Position of Ok button in game (white space)
okX := 2235
okY := 720
...
{
	;Make sure to uncomment below line when not testing in solo
	...
	xCheck := xStart + (okX * xMult)
	yCheck := yStart + (okY * yMult)
	...
	PixelSearch, outX, outy, xCheck-err, yCheck-err, xCheck+err, yCheck+err, %okayColor%, var, Fast
	while(ErrorLevel != 0) {
		Sleep, 1000
		PixelSearch, outX, outy, xCheck-err, yCheck-err, xCheck+err, yCheck+err, %okayColor%, var, Fast
	}

	Sleep, 2000 ; Wait 2 seconds for menu to be available
